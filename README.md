...
- `--gemini_model`: The Gemini model to use for summarization (default: `gemini-3.6-flash`).
- `--output`: Path to save the summary to a Markdown file.

Example:
```bash
python convocatcher.py --gui --model_size small --gemini_model gemini-3.6-pro
```
